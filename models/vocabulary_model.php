...
class vocabulary_Model extends Model {
    ...
    function qry_all_vocabulary_by_lesson($lesson_id) {
        ...
        $sql = "SELECT * FROM (SELECT * FROM t_vocabulary WHERE FK_LESSON = ?) vo"
                . " LEFT JOIN ( SELECT * FROM t_starred WHERE FK_USER = ?) st"
    ...
    function toggle_star() {
        ...
        $user_id = about_user::current_user()->user_id;
        $result = DB::search('t_starred', array(), array(
                    'FK_USER' => $user_id,
                    'FK_VOCABULARY' => $vocab_id
        ));
        if ($result) {
            DB::delete('t_starred', 'FK_USER = ? AND FK_VOCABULARY = ?', array($user_id, $vocab_id));
            return 0;
        } else {
            DB::insert('t_starred', array(
                'FK_USER' => $user_id,
                'FK_VOCABULARY' => $vocab_id
            ));
            return 1;
